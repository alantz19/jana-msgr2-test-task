          ...
          <li>
            <ul role="list" class="-mx-2 space-y-1">
              <li v-for="item in appStore.navigation" :key="item.name + item.current">
                ...
                <div v-else>
                  <button @click="item.current = !item.current"
                  ...
                  <ul class="mt-1 px-2" v-show="item.current">
                    <li v-for="subItem in item.children" :key="subItem.name">
                      <Link :href="subItem.href">
                        <div :class="[subItem.current ? 'bg-gray-50' : 'hover:bg-gray-50', 'block rounded-md py-2 pr-2 pl-9 text-sm leading-6 text-gray-700']">
                          {{ subItem.name }}
                        </div>
                      </Link>
                    </li>
...
router.on('navigate', (event) => {
  appStore.navigation = [
    ...
    },
    {
      ...
      href: '#',
      icon: CalendarIcon,
      current: event.detail.page.url.startsWith('/sms/routing/'),
      children: [
        {
          name: 'Companies',
          href: '/sms/routing/companies',
          current: event.detail.page.url.startsWith('/sms/routing/companies')
        },
